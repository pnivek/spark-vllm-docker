...
# DSV4 runtime mods (toggle via VLLM_MODS env at container start;
# see mods/run_mods.sh). Baked into the image so no host-side files needed.
COPY mods /opt/mods

# DSV4 entrypoint: applies $VLLM_MODS (no-op when unset) then execs the
# command. Keeps Deployment commands as plain `vllm serve ...` (Komodo
# splits command strings on &&, so the loader must not be chained there).
ENTRYPOINT ["/opt/mods/dsv4-entrypoint.sh"]
